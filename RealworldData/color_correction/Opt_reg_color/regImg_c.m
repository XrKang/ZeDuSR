function [I2warp,I2warp_color,tau,residue,OmegaOut] = regImg_c(I1,I2,tau,weight,maxIts)
...
%     [I2warp_lum,lum]=luminance_transfer(I1,I2warp,ind);
    [I2warp_color]=color_transfer(I1,I2warp,ind);
%     I2warp_lum=lum(1)*I2warp+lum(2);
    % get derivatives

    %% compute least square solution
    y = reshape(I1-I2warp_color,[sizeD,1]);

    X=getJacob(I2warp_color,tau);
    % compute X(i,:) = [I2x,I2y]*Jacob(d[x;y]/d tau)
    %X=-X;
    % solve
%   Weight=weight;
    Weight=weight.*repmat((OmegaOut(:)==0),[sizeI(3),1]);
...
function X=getJacob(I,tau)
    ...
    xCoord=repmat(xCoord(:),[sizeI(3),1]);
    yCoord=repmat(yCoord(:),[sizeI(3),1]);
