...
switch length(tau)
    case 2
        if mode == 0 
            Coordaft =[Coordb4(1,:)+tau(1);Coordb4(2,:)+tau(2)];
        else
            Coordaft =[Coordb4(1,:)-tau(1);Coordb4(2,:)-tau(2)];
        end        
    case 3
...
Xaft = reshape(Coordaft(1,:)',[sizeI(1),sizeI(2)]) + round(sizeI(1)/2);
Yaft = reshape(Coordaft(2,:)',[sizeI(1),sizeI(2)]) + round(sizeI(2)/2);

% if move out of the image plane
Omega = false([sizeI(1),sizeI(2)]);
...
if isempty(extrapval)
    ...
    for c=1:size(I,3)
    Iwarp(:,:,c) = interp2(Yb4,Xb4,I(:,:,c),Yaft,Xaft,'*bicubic');
    end
else
    for c=1:size(I,3)
    Iwarp(:,:,c) = interp2(Yb4,Xb4,I(:,:,c),Yaft,Xaft,'*bicubic',extrapval);
    end
end